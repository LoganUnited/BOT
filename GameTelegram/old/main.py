...
CHAT_ID_WHITELIST = [123456789, 987654321]  # Замените на реальные Chat ID
OPENWEATHER_API_KEY = "ВАШ_API_КЛЮЧ_OPENWEATHERMAP"  # Зарегистрируйтесь на openweathermap.org
MOSCOW_TIMEZONE_OFFSET = 3  # Часовой пояс Москвы (UTC+3)
...
def migrate_db(conn: sqlite3.Connection) -> None:
    ...
    for i in range(current_version, DB_VERSION):
        if i < len(migrations):
            ...
            print(f"⚠️ Нет миграции для версии {i + 1}")
...
# Декоратор для обновления активности
def update_activity(func):
    async def wrapper(update: Update, context: CallbackContext):
        user_id = update.message.from_user.id
        with sqlite3.connect('game.db') as conn:
            cursor = conn.cursor()
            cursor.execute('UPDATE users SET last_active = CURRENT_TIMESTAMP, is_afk = 0 WHERE user_id = ?', (user_id,))
            conn.commit()
        
        # Обновляем потребности
        needs.update_needs(user_id)  # Правильный вызов функции из модуля needs.py
...
async def show_profile(update: Update, context: CallbackContext) -> None:
    ...
    # Получаем текущие показатели потребностей
    needs = get_needs(user_id)
    if needs:
        food = needs["food"]
        water = needs["water"]
        sleep = needs["sleep"]
    ...
    try:
        with sqlite3.connect('game.db') as conn:
            ...
            for user_id, skills, last_active_str, is_afk in cursor.fetchall():
                ...
                # Начисление опыта только активным игрокам
                cursor.execute('''
                    UPDATE users SET 
...
async def eat(update: Update, context: CallbackContext) -> None:
    user_id = update.message.from_user.id
    if restore_need(user_id, "food", 20):  # Восстанавливаем 20% еды
...
async def drink(update: Update, context: CallbackContext) -> None:
    user_id = update.message.from_user.id
    if restore_need(user_id, "water", 20):  # Восстанавливаем 20% воды
...
async def rest(update: Update, context: CallbackContext) -> None:
    user_id = update.message.from_user.id
    if restore_need(user_id, "sleep", 20):  # Восстанавливаем 20% сна
        await update.message.reply_text("😴 Вы отдохнули! +20% к сну.")
    else:
